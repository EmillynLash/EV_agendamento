...
function renderCalendar(year, month) {
    ...
    const tituloMes = new Date(year, month).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
    ...
    for (let dia = 1; dia <= diasNoMes; dia++) {
        ...
        if (data < new Date(now.toDateString())) {
            ...
            diaEl.addEventListener('click', () => selectDate(data));
        }
...
confirmBtn.addEventListener('click', () => {
    ...
    const msg = `Olá, meu nome é ${nome}. Agendei um horario no dia ${dia} às ${selectedTime}h`;
